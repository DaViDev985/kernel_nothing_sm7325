...
function exports() {
    ...
    export KBUILD_BUILD_USER=P.A.N.Z.
...
function choices() {
    ...
    case $KSU_RESP in
         ...
         *)
            if [ $(grep -c $VERSION arch/arm64/configs/$DEFCONFIG) -eq 0 ]; then
                sed -i "s/-Spacewar/-Spacewar-$VERSION/" arch/arm64/configs/$DEFCONFIG
            fi
            ;;
    esac
    ...
    if [ $SIGINT_DETECT -eq 1 ]; then
        if [ $(grep -c "KSU" arch/arm64/configs/$DEFCONFIG) -ne 0 ]; then
            sed -i "s/-Spacewar-$VERSION-KSU/-Spacewar/" arch/arm64/configs/$DEFCONFIG
        fi
        if [ $(grep -c $VERSION arch/arm64/configs/$DEFCONFIG) -ne 0 ]; then
            sed -i "s/-Spacewar-$VERSION/-Spacewar/" arch/arm64/configs/$DEFCONFIG
        fi
        exit
    fi
}
...
function compile() {
    ...
    # KernelSU
    if [ $ZIPNAME = Spacewar-KernelSU ]; then
        sed -i 's/CONFIG_KSU=y/# CONFIG_KSU is not set/g' out/.config
        sed -i '/CONFIG_KSU=y/d' out/defconfig
        sed -i "s/-Spacewar-$VERSION-KSU/-Spacewar/" out/defconfig out/.config arch/arm64/configs/$DEFCONFIG
        ...
        sed -i "s/-Spacewar-$VERSION/-Spacewar/" out/defconfig out/.config arch/arm64/configs/$DEFCONFIG
    fi
